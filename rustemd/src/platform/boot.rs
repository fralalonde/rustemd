...
//! configuration.
//!
//! Only compiled with the `boot` feature, and only meaningful when rustemd is
...
/// `/sys` before `/sys/fs/cgroup` (which rustemd's cgroup code depends on).
///
...
pub fn early_boot() {
    if let Err(e) = set_hostname() {
        eprintln!("rustemd boot[hostname]: {e}");
    }
    if let Err(e) = ensure_machine_id() {
        eprintln!("rustemd boot[machine-id]: {e}");
    }
    if let Err(e) = apply_sysctl() {
        eprintln!("rustemd boot[sysctl]: {e}");
    }
    if let Err(e) = load_modules() {
        eprintln!("rustemd boot[modules]: {e}");
    }
    if let Err(e) = seed_random() {
        eprintln!("rustemd boot[random-seed]: {e}");
    }
    if let Err(e) = ensure_runtime_dirs() {
        eprintln!("rustemd boot[tmpfiles]: {e}");
    }
    if let Err(e) = mount_fstab() {
        eprintln!("rustemd boot[fstab]: {e}");
    }
}
...
/// stable id; rustemd's manager reads it for `%m` and identity).
...
fn seed_random() -> Result<(), String> {
    let seed = match fs::read("/var/lib/rustemd/random-seed") {
...
/// Minimal tmpfiles: the directories rustemd itself needs before it binds its
/// control/notify sockets.
fn ensure_runtime_dirs() -> Result<(), String> {
    for d in ["/run/rustemd", "/var/lib/rustemd", "/var/log"] {
