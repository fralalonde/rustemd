...
impl Drop for Daemon {
    fn drop(&mut self) {
        if let Ok(client) = rustemd::client::Client::for_mode(true) {
...
fn user_daemon_subprocess_accepts_named_pipe_control() {
    ...
    let mut child = std::process::Command::new(env!("CARGO_BIN_EXE_rustemd"))
        .args(["daemon", "--user"])
        .env("RUSTEMD_UNIT_PATH", scratch.dir.path().join("units"))
        .env("RUSTEMD_CONFIG_DIR", scratch.dir.path().join("config"))
        .env("RUSTEMD_RUNTIME_DIR", scratch.dir.path().join("runtime"))
    ...
    assert!(wait_for(Duration::from_secs(5), || client
        ...
        .is_ok_and(|state| state == ["active"])));
    rustemd::client::Client::for_mode(true)
        .unwrap()
        .simple_op("shutdown")
        .unwrap();
...
fn named_pipe_bind_rejects_an_existing_endpoint() {
    let scratch = Scratch::new();
    let first = rustemd::platform::net::bind_control(&scratch.pipe).unwrap();
    let second = rustemd::platform::net::bind_control(&scratch.pipe);
    assert!(
        second.is_err(),
