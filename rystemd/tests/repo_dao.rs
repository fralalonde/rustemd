...
//! `rystemd::repo` DAO, and that the `repo` control op lets a client discover
//! and reopen the same repository.
//!
//! The unit file is written with `rystemd::repo::Repo` (the same way a client
...
/// Serializes `RYSTEMD_*` env mutation across tests in this binary.
...
fn daemon_lists_and_reads_units_through_repo_dao() {
    ...
    let repo = rystemd::repo::Repo::open_roots(vec![scratch.units()]).unwrap();
    let definition = rystemd::repo::UnitDefinition::parse(
        "dao.service",
    ...
    let client_repo = rystemd::repo::Repo::open(std::path::PathBuf::from(&info.root)).unwrap();
