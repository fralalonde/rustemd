...
fn cli_drives_windows_user_manager_over_named_pipe() {
    ...
    let pipe = format!(r"\\.\pipe\rystemd-cli-test-{}", std::process::id());
    unsafe {
    ...
    let manager = std::thread::spawn(|| {
        let mut manager =
            rystemd::manager::Manager::new(rystemd::manager::ManagerCfg::for_mode(true).unwrap())
                .unwrap();
        manager.load_all();
    ...
    let client = rystemd::client::Client::for_mode(true).unwrap();
    assert!(wait_for(Duration::from_secs(5), || client
        .simple_op("list_units")
        .is_ok()));

    let binary = env!("CARGO_BIN_EXE_rystemctl");
    let start = Command::new(binary)
        .args(["--user", "start", "cli.service"])
        .env("RYSTEMD_SOCKET", &pipe)
        .output()
        .unwrap();
    assert!(
