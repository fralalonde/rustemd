...
use rystemd::control::{Control, SocketClient, TimerInfo, UnitFileInfo, UnitStatus, UnitSummary};
...
pub fn connect(user: bool) -> Result<SocketClient, String> {
    ...
    ctl.get_default().map_err(|e| {
        ...
        format!("no {mode} manager reachable — start one with `rystemd daemon{flag}` ({e})")
    })?;
    Ok(ctl)
}
