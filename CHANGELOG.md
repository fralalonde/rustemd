...
All notable changes to rustemd are documented here. The format follows
[Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and versions are
derived from Git tags via `build.rs` (never hardcoded).

## [0.1.0] — Unreleased

Initial release of rustemd: a systemd / `systemctl` reimplementation in Rust.

### Added

- **Unit manager daemon (`rustemd`)** with an internal job engine, dependency
...
- **`rustemctl`** — a `systemctl`-compatible CLI: `start`/`stop`/`restart`/
  ...
  shell completions.
- **`rustemd-tui`** — a terminal UI client for a running manager.
...
- **D-Bus (opt-in `dbus` feature):** a native `org.rustemd.Manager1` interface
...
- **Packaging:** Homebrew (`fralalonde/rustemd` tap), `.deb` / `.rpm`, portable
